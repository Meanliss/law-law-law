document.addEventListener('DOMContentLoaded', () => {
  ...
  // ===== HELPER FUNCTION: Smart Source Text Trimming =====
  function trimSourceText(sourceText) {
    /**
     * Intelligently extract law name and document number from source text
     * Input: "Luật Hôn nhân và gia đình Luật Hôn nhân và gia đình số 52/2014/QH13 ngày 19 tháng 6 năm 2014..."
     * Output: "Luật Hôn nhân và gia đình số 52/2014/QH13"
     */
    
    // Pattern 1: Match "Luật [Name] số [Number]"
    const pattern1 = /^(Luật[^,]+?)\s+(số\s+\d+\/\d+\/[A-Z0-9]+)/i;
    const match1 = sourceText.match(pattern1);
    if (match1) {
      // Remove duplicate law name if present
      let lawName = match1[1];
      const words = lawName.split(/\s+/);
      
      // Check if law name appears twice (e.g., "Luật ABC Luật ABC")
      ...
      return `${lawName} ${match1[2]}`;
    }
    
    // Pattern 2: Match "Nghị định [Name] số [Number]"
    const pattern2 = /^(Nghị định[^,]+?)\s+(số\s+\d+\/\d+\/[A-Z\-0-9]+)/i;
    const match2 = sourceText.match(pattern2);
    if (match2) {
      return `${match2[1]} ${match2[2]}`;
    }
    
    // Pattern 3: Match any document type with số [Number]
    ...
    // Fallback: If no pattern matches, take first 100 characters
  ...
  function addMessage(text, sender, save = true, animated = true, metadata = null, messageIndex = -1) {
    ...
    if (sender === 'bot' && metadata) {
      ...
      // Hiển thị sources với clickable cards
      if (metadata.sources && metadata.sources.length > 0) {
        ...
        if (metadata.pdf_sources && metadata.pdf_sources.length > 0) {
          metadata.pdf_sources.forEach((pdfSource, idx) => {
            pdfSourcesByIndex[idx] = pdfSource;
          });
        }

        // Add clickable source citations with trimmed text
        metadata.sources.slice(0, 3).forEach((s, i) => {
